...
class GlobalOptions(BaseModel):
    ...
    auth_file: str = Field(..., description="Path to authentication/credentials JSON file")
    log_db_auth_id: str = Field(..., description="Credential ID for the log database connection")
...
class LogdbTruncateParams(GlobalOptions):
    ...
    sync_id: Optional[str] = Field(None, description="Truncate only data for this sync_id")
...
class LogdbReleaseLocksParams(GlobalOptions):
    ...
    max_age_hours: Optional[int] = Field(None, description="Release locks older than this many hours")
    table_id: Optional[str] = Field(None, description="Release lock for a specific table ID")
...
class ConfigCreateParams(GlobalOptions):
    ...
    source_schema_name: Optional[str] = Field(None, description="Comma-separated list of source schemas")

    # Filtering
    include: Optional[str] = Field(None, description="Table include patterns (comma-separated, supports wildcards)")
    exclude: Optional[str] = Field(None, description="Table exclude patterns (comma-separated, supports wildcards)")
    min_rows: Optional[int] = Field(None, description="Minimum row count for table inclusion")
    max_rows: Optional[int] = Field(None, description="Maximum row count for table inclusion")

    # Incremental
    incremental_table: Optional[List[str]] = Field(None, description="Incremental table config: schema.table:column:type[:i|:e][@start][!strategy]")
    incremental_safety_lag: Optional[int] = Field(None, description="Safety lag in seconds for incremental exports")

    # Storage
    output_dir: Optional[str] = Field(None, description="Local output directory for exports")
    target_storage_id: Optional[str] = Field(None, description="Target storage credential ID (for cloud storage)")
    sub_path: Optional[str] = Field(None, description="Sub-path within storage location")

    # FastBCP
    fastbcp_dir_path: Optional[str] = Field(None, description="Path to FastBCP binary directory")
    fastbcp_p: Optional[int] = Field(None, description="FastBCP parallel degree")
    n_jobs: Optional[int] = Field(None, description="Number of concurrent table exports")
    compression_type: Optional[CompressionType] = Field(None, description="Parquet compression type")
    large_table_threshold: Optional[int] = Field(None, description="Row threshold for large table handling")
    fastbcp_table_config: Optional[str] = Field(None, description="Per-table FastBCP config: 'table1:method:key:degree;table2:method:key:degree'")

    # Publishing
    publish_target: Optional[str] = Field(None, description="Publish target credential ID")
    publish_method: Optional[PublishMethod] = Field(None, description="Publish method (external tables or internal/loaded)")
    publish_database_name: Optional[str] = Field(None, description="Target database name for publishing")
    publish_schema_pattern: Optional[str] = Field(None, description="Schema naming pattern for publishing")
    publish_table_pattern: Optional[str] = Field(None, description="Table naming pattern for publishing")

    # Features
    no_views: bool = Field(False, description="Disable view creation when publishing external tables")
    pk_constraints: bool = Field(False, description="Enable primary key constraints when publishing")
    ...
    # Other
    sync_id: Optional[str] = Field(None, description="Custom sync ID (1-64 chars, alphanumeric/underscore/hyphen)")
    error_action: Optional[ErrorAction] = Field(None, description="Action on error: fail, continue, or skip")
    env_name: Optional[str] = Field(None, description="Environment name for configuration isolation")
    ...
    def validate_publish_method_requires_target(self):
        ...
        if self.publish_method and not self.publish_target:
            raise ValueError(
                "publish_method requires publish_target to be set."
            )
        return self
...
class SyncParams(BaseModel):
    ...
    run_id: Optional[str] = Field(None, description="Specific run_id to resume or continue")
    auth_file: Optional[str] = Field(None, description="Override auth file")
    fastbcp_dir_path: Optional[str] = Field(None, description="Override FastBCP location")
    log_level: Optional[LogLevel] = Field(None, description="Logging verbosity level")
    log_dir: Optional[str] = Field(None, description="Directory for log files")
    no_progress: bool = Field(False, description="Disable progress bar display")
    no_banner: bool = Field(False, description="Suppress the startup banner")
...
class RunParams(BaseModel):
    ...
    auth_file: Optional[str] = Field(None, description="Path to authentication JSON file (overrides YAML)")
    log_db_auth_id: Optional[str] = Field(None, description="Log database credential ID (overrides YAML)")
    log_level: Optional[LogLevel] = Field(None, description="Logging verbosity level")
    log_dir: Optional[str] = Field(None, description="Directory for log files")
    no_progress: bool = Field(False, description="Disable progress bar display")
    no_banner: bool = Field(False, description="Suppress the startup banner")
...
class StatusParams(GlobalOptions):
    ...
    sync_id: Optional[str] = Field(None, description="Show runs for this sync configuration")
...
class CleanupParams(GlobalOptions):
    ...
    older_than: Optional[str] = Field(None, description="Only delete runs older than this duration (e.g., 7d, 24h, 30m)")
    status: Optional[CleanupStatus] = Field(None, description="Only delete runs with this status")
...
class LakeXpressRequest(BaseModel):
    ...
    logdb_init: Optional[LogdbInitParams] = Field(None, description="Parameters for logdb init")
    logdb_drop: Optional[LogdbDropParams] = Field(None, description="Parameters for logdb drop")
    logdb_truncate: Optional[LogdbTruncateParams] = Field(None, description="Parameters for logdb truncate")
    logdb_locks: Optional[LogdbLocksParams] = Field(None, description="Parameters for logdb locks")
    logdb_release_locks: Optional[LogdbReleaseLocksParams] = Field(None, description="Parameters for logdb release-locks")
    config_create: Optional[ConfigCreateParams] = Field(None, description="Parameters for config create")
    config_delete: Optional[ConfigDeleteParams] = Field(None, description="Parameters for config delete")
    config_list: Optional[ConfigListParams] = Field(None, description="Parameters for config list")
    sync: Optional[SyncParams] = Field(None, description="Parameters for sync")
    sync_export: Optional[SyncExportParams] = Field(None, description="Parameters for sync[export]")
    sync_publish: Optional[SyncPublishParams] = Field(None, description="Parameters for sync[publish]")
